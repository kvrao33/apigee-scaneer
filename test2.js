  ...
  worksheetData.forEach((row, rowIndex) => {
    if (row[0] === "Line: Column") {
      ...
      if (
        ...
        !isDigitColonDigit &&
        firstCell !== "Name" &&
        !firstCell.includes("Apigee Lint Report:")
      ) {
        // Define the merge range for columns A and B
        worksheet["!merges"] = worksheet["!merges"] || [];
        worksheet["!merges"].push({
          s: { r: rowIndex, c: 0 }, // Start cell (A of current row)
          e: { r: rowIndex, c: 1 }, // End cell (B of current row)
        });
        worksheet["!cols"] = [
          { wch: 12 }, // Column A
          { wch: 50 }, // Column B
        ...
        worksheet["!rows"] = worksheetData.map((_, index) => ({
          hpt: index === 0 ? 30 : 20, // Example heights: 30pt for header, 20pt for others
        }));
        ...
        // Apply yellow background fill to columns A to D
        ["A", "B", "C", "D"].forEach((col) => {
          const cell = `${col}${rowIndex + 1}`;
          applyStyleIfExists(worksheet, cell, yellowBackground);
        });
      }
    }
  });
